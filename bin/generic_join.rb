...
unless file_pat and file_pat.size > 0
  STDERR.puts("Please supply a base filename.")
  exit(1)
end

unless prefix and prefix.size > 0
  STDERR.puts("Please supply a prefix")
  exit 1
end
...
style.add_attribute("href", "file:///home/jmoses/book_library/style.css")
head.add_element("title").add_text("#{prefix}_full.html")
      ...
      doc.elements["html/body"].elements.each do |e|
        case e.name
          ...
          when 'p'
            ...
            if e.text.strip == "" and e.children.size == 1
...
File.open("#{prefix}_full.html", 'w') do |output|
